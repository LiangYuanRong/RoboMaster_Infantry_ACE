...
{	 
  ...
  GPIO_Init(GPIOF, &GPIO_InitStructure);//初始化
	...
	GPIO_PinAFConfig(GPIOF,GPIO_PinSource8,GPIO_AF_SPI5); //PF8复用为 SPI5
	GPIO_PinAFConfig(GPIOF,GPIO_PinSource9,GPIO_AF_SPI5); //PF9复用为 SPI5
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_128;		//定义波特率预分频的值:波特率预分频值为128    决定传送速率84M/128=656.25KHz
